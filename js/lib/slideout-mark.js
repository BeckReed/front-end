...
function Slideout(options) {
  ...
  this._padding = this._translateTo = parseInt(options.padding, 10) || 256;//nav面板大小
  this._orientation = this._side === 'right' ? -1 : 1;//导航nav面板显示的方向
...
Slideout.prototype.open = function() {
  var self = this;
  this.emit('beforeopen');//触发用户自定义的beforeopen事件
  /*
  添加overflow hidden样式 和固定侧边栏的位置
  */
  if (!html.classList.contains('slideout-open')) {
    html.classList.add('slideout-open');
  }
  this._setTransition();//设置侧滑时间的过渡效果
  this._translateXTo(this._translateTo);//设置样式tranlateX位置偏移
  this._opened = true;
  setTimeout(function() {
    self.panel.style.transition = self.panel.style['-webkit-transition'] = '';//移除之前的过渡效果
    self.emit('open');////触发用户自定义的open事件
  }, this._duration + 50);
  return this;
};
...
Slideout.prototype.close = function() {
  ...
  this.emit('beforeclose');//触发关闭前的自定义事件
  this._setTransition();
  this._translateXTo(0);//重新还原位移到初始化状态
  this._opened = false;
  setTimeout(function() {
    html.classList.remove('slideout-open');
    self.panel.style.transition = self.panel.style['-webkit-transition'] = self.panel.style[prefix + 'transform'] = self.panel.style.transform = '';
    self.emit('close');//用户自定义的open事件
  }, this._duration + 50);
  return this;
};
...
Slideout.prototype.toggle = function() {
  return this.isOpen() ? this.close() : this.open();//根据打开关闭的状态切换侧滑菜单
};
...
Slideout.prototype._initTouchEvents = function() {
  ...
  this._onScrollFn = decouple(doc, 'scroll', function() {
    //https://www.npmjs.com/package/decouple  decouple去耦 nodejs包  事件绑定
    if (!self._moved) {
  ...
  this.panel.addEventListener(touch.start, this._resetTouchFn);//touchstart事件绑定
  ...
  this._onTouchMoveFn = function(eve) {
    ...
    console.log('clientX: '+ eve.touches[0].clientX);
    console.log('startOffsetX: '+ self._startOffsetX);

    var dif_x = eve.touches[0].clientX - self._startOffsetX;//获取移动的大小的距离
    var translateX = self._currentOffsetX = dif_x;
    console.log(translateX);
      console.log(self._padding);
    ...
    if (Math.abs(dif_x) > 20) {
      ...
      console.log('move-oriented-dif-x: '+oriented_dif_x);
      console.log(self._opened );

      if (self._opened && oriented_dif_x > 0 || !self._opened && oriented_dif_x < 0) {
        /*
          1.通过点击按钮出现导航条再像右移动时不再做任何移动
          2.导航条已经隐藏，并且同时向左移动时不再做任何移动
        */
        return;
      }
